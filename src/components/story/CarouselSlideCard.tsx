...
function Emph({ text }: { text: string }) {
  ...
  return <>{parts.map((p, i) => (i % 2 ? <span key={i} style={{ fontWeight: 700 }}>{p}</span> : p))}</>;
}
      ...
      return (
        <div ref={ref} className={`relative ${aspect} overflow-hidden select-none`} style={{ ...base, backgroundColor: BRAND.paper }}>
          ...
          <div className="absolute flex items-center" style={{ left: "8cqw", right: "8cqw", top: format === "square" ? "22cqw" : "30cqw", height: format === "square" ? "56cqw" : "52cqw", backgroundColor: BRAND.red, clipPath: BUBBLE_TAIL }}>
            <h2 style={{ ...headlineStyle("9.6cqw"), color: BRAND.paper, padding: "0 7cqw" }}>
          ...
          {slide.kicker && (
            <div
              className="absolute"
              style={{ ...kickerStyle("3.2cqw", BRAND.paper), left: "12cqw", top: format === "square" ? "84cqw" : "90cqw", padding: "1.7cqw 3.2cqw", backgroundColor: BRAND.blue, clipPath: BUBBLE_CHIP }}
            >
              {slide.kicker}
            </div>
          )}
          <div className="absolute" style={{ bottom: "4.5cqw", right: "8.5cqw", fontFamily: SERIF, fontWeight: 700, fontSize: "4.2cqw", color: BRAND.purple }}>politikradar.</div>
          <div className="absolute" style={{ bottom: "5cqw", left: "12cqw", fontFamily: SANS, fontWeight: 700, fontSize: "2.8cqw", color: BRAND.purple }}>{total} Slides → wischen</div>
        </div>
      );
    }
    ...
    if (slide.slide_type === "detail") {
      return (
        <div ref={ref} className={`relative ${aspect} overflow-hidden select-none`} style={{ ...base, backgroundColor: BRAND.blue, color: BRAND.greenBright }}>
          ...
          <div className="absolute text-center w-full" style={{ top: "7cqw", fontFamily: SERIF, fontWeight: 700, fontSize: "3.6cqw" }}>politikradar.</div>
          <div className="absolute" style={{ left: "10cqw", right: "10cqw", top: "17cqw" }}>
            {slide.kicker && <div style={{ ...kickerStyle("2.9cqw"), opacity: 0.9 }}>{slide.kicker}</div>}
            <h2 style={{ ...headlineStyle("8.8cqw", 1.06), margin: "2.6cqw 0 0" }}><Emph text={slide.headline} /></h2>
            <div style={{ width: "10cqw", height: "0.8cqw", backgroundColor: BRAND.greenBright, margin: "4cqw 0" }} />
            {slide.body && (
              <p style={{ ...(bodyIsQuote ? quoteStyle("4.4cqw") : bodyStyle("3.8cqw")), maxWidth: "72cqw" }}>{slide.body}</p>
            )}
          </div>
          {slide.hashtags?.length ? (
            <div className="absolute flex flex-wrap" style={{ left: "10cqw", bottom: "11cqw", gap: "1.7cqw" }}>
              {slide.hashtags.map((h) => (
                <span key={h} style={{ ...hashtagPill("3.4cqw", BRAND.greenSoft, BRAND.green), padding: "1.6cqw 3cqw" }}>#{h.replace(/^#/, "")}</span>
              ))}
            </div>
          ) : null}
        </div>
    ...
    if (slide.slide_type === "result") {
      ...
      return (
        <div ref={ref} className={`relative ${aspect} overflow-hidden select-none`} style={{ ...base, backgroundColor: BRAND.yellow, color: BRAND.ink }}>
          <div className="absolute" style={{ top: "7cqw", left: "9cqw", fontFamily: SERIF, fontWeight: 700, fontSize: "3.6cqw" }}>politikradar.</div>
          {slide.kicker && <div className="absolute" style={{ ...kickerStyle("2.9cqw", BRAND.redDeep), top: "7.3cqw", right: "9cqw" }}>{slide.kicker}</div>}
          <div className="absolute" style={{ left: "9cqw", right: "9cqw", top: "18cqw" }}>
            <h2 style={headlineStyle("7cqw", 1.08)}><Emph text={slide.headline} /></h2>
          </div>
          <div
            className="absolute flex items-center justify-center"
            style={{
              ...resultBubble("5cqw", accepted ? BRAND.green : BRAND.redDeep, BRAND.paper),
              left: "9cqw",
          ...
          <div className="absolute flex flex-col" style={{ left: "9cqw", right: "9cqw", top: format === "square" ? "68cqw" : "78cqw", gap: "3.3cqw" }}>
            {([["Ja", v.yes, BRAND.green], ["Nein", v.no, BRAND.red], ["Enthaltungen", v.abstain, BRAND.purple]] as const).map(([label, n, color]) => (
              <div key={label}>
                <div className="flex justify-between" style={{ fontFamily: SANS, fontWeight: 800, fontSize: "3.6cqw", marginBottom: "1.4cqw" }}><span>{label}</span><span className="tabular-nums">{n}</span></div>
    ...
    return (
      <div ref={ref} className={`relative ${aspect} overflow-hidden select-none`} style={{ ...base, backgroundColor: BRAND.purple, color: BRAND.paper }}>
        ...
        <div className="absolute" style={{ left: "11cqw", right: "11cqw", top: format === "square" ? "22cqw" : "30cqw" }}>
          <h2 style={headlineStyle("10.4cqw")}><Emph text={slide.headline} /></h2>
          {slide.body && (
            <p style={{ ...(bodyIsQuote ? quoteStyle("4.8cqw") : bodyStyle("4.2cqw", 1.35)), maxWidth: "68cqw", marginTop: "3.3cqw" }}>{slide.body}</p>
          )}
        </div>
        <div
          className="absolute"
          style={{ ...ctaStyle("4.2cqw", BRAND.paper, BRAND.purple), left: "11cqw", top: format === "square" ? "66cqw" : "78cqw", padding: "2.4cqw 4cqw" }}
        >
          {slide.cta_label ?? "@politikradar folgen"}
        </div>
        <div className="absolute" style={{ bottom: "6cqw", left: "11cqw", fontFamily: SERIF, fontWeight: 700, fontSize: "4.2cqw" }}>politikradar.</div>
        <div className="absolute" style={{ bottom: "6.6cqw", right: "11cqw", fontFamily: SANS, fontWeight: 700, fontSize: "2.8cqw", opacity: 0.85 }}>politikradar.mathiasmenzl.ch</div>
      </div>
    );
  }
);
